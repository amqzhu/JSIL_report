...
\subsection{Testing}
\az{need to figure out the exact numbers of TCs: Total, Passed, Failed, Aborted, Timeout for each of Object.prototype, Array, and String, plus ES5 vs ES6 tests}
...
Extensive changes have occurred in the 5\textsuperscript{th} edition of the ECMAScript standard, including the addition of a strict mode and accessor properties\cite{EcmaScript} , both of which will be addressed in this project. Strict mode has different semantics from non-strict JavaScript, with more explicit error reporting and better behaviors in general. Accessor properties are namely getters and setters, which are functions that retrieve or store the property value for objects. This chapter serves as an in-depth introduction to the internal workings of JavaScript as a programming language.
...
The parentheses, \jsinline|( ... )|, serve to group the first set of symbols separately from the rest. Within the parentheses, there are two groups of empty square brackets \jsinline|[]|, although they get coerced into different types. The exclamation mark that represents \textit{not}, binds more closely than the \jsinline|+| symbol, thus, \jsinline|![]| is taken as a unit before the rest is evaluated. \jsinline|[]| represents an empty array in JavaScript, and taking the negation of the empty array coerces it into the boolean value \textbf{false}. Normally, the plus operator, \jsinline|+|, is used to coerce any type of objects into numbers, so \jsinline|+[]| would become \textbf{0}, since arrays are objects. In this case, however, \jsinline|+| acts as the string concatenation operator instead, connecting the boolean value \textbf{false} and an empty array. The empty array is coerced into an empty string, and the expression becomes the string value \jsinline|"false"|. The second group of symbols are within a group of square brackets, in this case, functions as the indexing operator on the string \jsinline|"false"|. As discussed, \jsinline|+[]| indeed gets coerced to the integer value \textbf{0} here, but as the negation symbol \jsinline|![]| precedes it, it is treated as the boolean value \textbf{false}, and \jsinline|!0| becomes the boolean value \textbf{true}. Lastly, \jsinline|+true| once again coerces the expression into an integer, which has the value \textbf{1}. The entire expression then becomes
\begin{center}
\jsinline|"false"[1]|
\end{center}
...
Function Objects can be called in one of three ways. The following code snippet produces the heap structure shown in Figure 3.3.
\begin{adjustwidth}{4cm}{4cm}
\begin{lstjs}
...
\begin{tikzpicture}
  ...
  \node[sbox,sz={\d}{\w},rectangle split,rectangle split parts=3, fill=black!35] (global) 
    {     
      \nodepart{one}{\scriptsize prototype: \(l_{\mathsf{objproto}}\)}
  ...
  %%%%%%%% end of op 
  %%%%%%%% connection 
  \draw[p] ($(global.second) + (1.8*\w,0.75cm)$) to[out=0,in=180] (op);
  \draw[p] ($(global.three) + (1.8*\w,0)$) to[out=0,in=180] (number);
  \draw[p] ($(np.second) + (1.8*\w,0)$) to[out=0,in=0] (op);
  \draw[p] ($(number.second) + (1.8*\w,0)$) to [out=0,in=180]  (np);
  %%%%%%%% end connection
\end{tikzpicture}
\caption{Heap of code snippet above}
\label{fig:initheap}
\end{center}
\end{figure}

\begin{description}
\item[Function Call] In a function call in strict mode, the \jsinline|this| value would be bound to \textbf{undefined}, so when evaluating \jsinline|this.age| from the calcAge() function, the result would be \textbf{undefined}.
\item[Method Call] When the function is used as a method call, \jsinline|this| is bound to the global object, so \jsinline|this.birthYear| would be evaluated to 1990, and \jsinline|this.age| would be 2016 - 1990, which equals 26.
\item[Constructor Call] When the function is used in a constructor call, such as \jsinline|new calcAge()|, \jsinline|this| is bound to the object created by the constructor. This new object would have the function prototype object as its prototype, therefore \jsinline|this.birthYear| would evaluate to 1980, and \jsinline|this.age| would be 36, from 2016 - 1980.
\end{description}
