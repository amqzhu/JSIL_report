...
This project extends the implementation of JSIL to include various built-in methods in the Object.prototype, Array, and String libraries, as well as the arguments object construct. The specification in the 5\textsuperscript{th} edition of the ECMAScript standard has been faithfully followed during JSIL implementation to ensure correctness. When the specification is vague and up to interpretation, operational semantics and pseudocode are utilized to ensure clarity of method actions. All JSIL code written is concurrently tested first using manually constructed unit tests, before using an automatic testing framework, which then lends itself to manual analysis. The testing framework pulls test cases from the official ECMAScript test suite, Test262, and all methods implemented have passed the 2263 test cases within the scope of this project. The completion of this project signaled a 40\% increase in test coverage within the conformance test suite.
\end{abstract}
...
\and
\inferrule[While-2(false) (2b)]{\mathtt{false}(v)}{\vdash \langle h, \textnormal{while}_2(v,e) \{ s, o \} \rangle \Downarrow \langle h, o \rangle}
\and
\inferrule[While-2(true) (2c)]{\neg \mathtt{false}(v) \\ \vdash \langle h, s \rangle \Downarrow \langle h_1, o_1 \rangle \\\\ \vdash \langle h, \textnormal{while}_3(e) \{ s,o,o_1 \} \rangle \Downarrow \langle h_2, o_2 \rangle}{ \vdash \langle h, \textnormal{while}_2(v,e) \{ s,o \} \rangle \Downarrow \langle h_2, o_2 \rangle}
\and
...
%The initial phase of this project sought to complete the implementation of core strict mode, with core defined as all of the language constructs, together with library functions that tap into these constructs. All other library functions that could be defined using what is already in core were not included in this initial stage but were intended to be incorporated later on in the project. 
...
The following operational semantics written in pretty big step style, faithfully replicates the language of the ECMAScript standard for Object.prototype.isPrototypeOf(). The first rule determines the type of the argument provided, and the second rule returns \texttt{false} if the argument V is not an object, corresponding to step 1 in the standard. The next rule corresponds to step 2 in the standard, calling the \texttt{ToObject} built-in function on the \textbf{this} object. The next rule corresponds to the case of reaching the end of the prototype chain, in which case \texttt{false} is returned, as stated in steps 3a and 3b. The following rule corresponds to steps 3a and 3c, returning \texttt{true} when a prototype object is found, while the last rule takes care of the \textbf{Repeat} in the standard. 

\begin{mathpar}
\inferrule[$\mathcal{I}_{IPO}^{op}$-TypeOf (Step 1)]{L, v_t \vdash \langle h, \mathcal{I}_{typeOf}(v) \rangle \Downarrow \langle h_1, o_1 \rangle \\\\ L, v_t \vdash \langle h_1, \mathcal{I}_{IPO_1}(v, o_1) \rangle \Downarrow \langle h_f, o_f \rangle}{L, v_t \vdash \langle h, \mathcal{I}_{IPO}(v) \rangle \Downarrow \langle h_f, o_f \rangle}
\and
\inferrule[$\mathcal{I}_{IPO_1}$-False (Step 1)] {type \neq ``object"}{L, v_t \vdash \langle h_1, \mathcal{I}_{IPO_1}(v, type) \rangle \Downarrow \langle h_1, \mathtt{false} \rangle}
\and
\inferrule[$\mathcal{I}_{IPO}$-ToObject (Step 2)]{L, v_t \vdash \langle h_1, \mathcal{I}_{toObject}(v_t) \rangle \Downarrow \langle h_2, o_2 \rangle \\\\ L, v_t \vdash \langle h_2, \mathcal{I}_{IPO_2}(v, o_2) \rangle \Downarrow \langle h_f, o_f \rangle}{L, v_t \vdash \langle h_1, \mathcal{I}_{IPO_1}(v, ``object") \rangle \Downarrow \langle h_f, o_f \rangle}
\and
\inferrule[$\mathcal{I}_{IPO}$-Null (Step 3ab)]{h_2(v, @proto) = \mathtt{null}}{L, v_t \vdash \langle h_2, \mathcal{I}_{IPO_2}(v, l) \rangle \Downarrow \langle h_2, \mathtt{false} \rangle}
\and
\inferrule[$\mathcal{I}_{IPO}$-Same (Step 3ac)]{h_2(v, @proto) = l}{L, v_t \vdash \langle h_2, OP-IPO_2(v, l) \rangle \Downarrow \langle h_2, \mathtt{true} \rangle}
\and
\inferrule[$\mathcal{I}_{IPO}$-Repeat (Step 3)]{h_2(v, @proto) = l' \and l' \neq null \and l' \neq l \\\\ L, v_t \vdash \langle h_2, \mathcal{I}_{IPO_2}(l', l) \rangle \Downarrow \langle h_f, o_f \rangle}{L, v_t \vdash \langle h_2, \mathcal{I}_{IPO_2}(v, l) \rangle \Downarrow \langle h_f, o_f \rangle}
\end{mathpar}
...
\subsection{Operational Semantics}
The language in the ECMAScript standard for this method was rather clear, thus the operational semantics written in pretty big step style closely corresponds to the specification, and the steps are written out in the names of the rules. The optional thisArg argument, was modeled as \textit{ot} until the sixth rule, and the rules corresponding to steps 5-6a and 5-6b distinguish between the cases for when this optional argument is provided. In addition, multiple rules correspond to certain steps when boolean conditions are involved, such as in step 4 and step 9ciii.
...
\and
\inferrule[AP-F (Step 9ciii)]{L, v_t \vdash \langle h_6, \mathcal{I}_{ToBoolean}(selected) \rangle \Downarrow \langle h_{10}, o_{10} \rangle \\\\ L, v_t \vdash \langle h_{10}, AP-F_{10}(cf, thisArg, O, len, Arr, kval, selected, o_{10}, 0, 0) \rangle \Downarrow \langle h_f, o_f \rangle}{L, v_t \vdash \langle h_9, AP-F_9(cf, thisArg, O, len, Arr, kval, selected, 0, 0) \rangle \Downarrow \langle h_f, o_f \rangle}
\and
\inferrule[AP-F-False (Step 9ciii)]{L, v_t \vdash \langle h_{10}, AP-F_{Loop}(cf, thisArg, O, len, Arr, 1, 0) \rangle \Downarrow \langle h_f, o_f \rangle}{L, v_t \vdash \langle h_{10}, AP-F_{10}(cf, thisArg, O, len, Arr, kval, selected, false, 0, 0) \rangle \Downarrow \langle h_f, o_f \rangle}
\and
\inferrule[AP-F-Loop (Step 9ciii,9c)]{L, v_t \vdash \langle h_6, \mathcal{I}_{ToString}(0) \rangle \Downarrow \langle h_{11}, o_{11} \rangle \\\\ L, v_t \vdash \langle h_{11}, \mathcal{I}_{dop}(Arr, o_{11}, kval, true, true, true, false) \rangle \Downarrow \langle h_{12}, o_{12} \rangle \\\\ L, v_t \vdash \langle h_{12}, AP-F_{Loop}(cf, thisArg, O, len, Arr, 1, 1) \rangle \Downarrow \langle h_f, o_f \rangle}{L, v_t \vdash \langle h_{12}, AP-F_{10}(cf, thisArg, O, len, Arr, kval, selected, true, 0, 0) \rangle \Downarrow \langle h_f, o_f \rangle}
\end{mathpar}
...
The Array.prototype.sort(comparefn) method is undoubtedly the most complicated in the Array Library. If the array object falls into any of the four predefined categories, then an implementation defined algorithm will be used to sort the array. 
...
Since this project works exclusively in strict mode, the JSIL implemented corresponds to the strict mode algorithm, and is hence relatively simple. The implementation makes use of two instances of [[DefineOwnProperty]], on lines 12 and 19, each providing a data descriptor in the form of a JSIL list. The ``caller" and ``callee" properties are defined as accessor descriptors modeled as JSIL lists, setting the getter and setter as error throwing functions.
...
proc create_arguments_object (argList) { 
	...
	head:	xret := "i__toString" (indx) with elab;
			xret := dop (obj, xret, {{ "d", l-nth (argList, indx), $$t, $$t, $$t }}, $$f) with elab;
			indx := indx - 1;
			goto loop;
